<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	import="org.json.simple.JsonArray" import="siteEntities.Comrade"%>
<!DOCTYPE html>
<html>
<head>
...
<div class="init-body col-md-6">
	<div class="row">
			...
			<div class="card white">
				<div class="card-image">
					<img src="https://coverfiles.alphacoders.com/128/12862.jpg"
						style="height: 50vh"> <span class="card-title disclaimer">${groupName}
						${name}
						<%if ((String)request.getAttribute("iscom") == "isnot"){ %>
						<a href="PageServlet?joincom=y"class = "waves-effect waves-light btn black">Join</a></span>
						<%} %>
						<% if((String)request.getAttribute("iscom")!="isnot"){%>
						<a href="PageServlet?joincom=n"class = "waves-effect waves-light btn black">Leave</a></span>
						<%} %>
				</div>
			</div>
		</div>
		...
		<div class = "row">
			...
			<% JsonArray comrades = (JsonArray) request.getAttribute("members");
				for(int i = 0; i < comrades.size();i++){
					...
					%> <a href="ComradeServlet?ComradeAdd=<%out.println(name);%>"><%out.println(name);%></a><br>
					   <a href = "GroupServlet?Expel=<%out.println(name); %>">${expel}</a><br>
			<% } %>
					</div>
				</div>
			</div>
		...
		<%if (request.getAttribute("notMember") != null){ %>
			<div class="card white">
				<div class="card-content">
					<span class="card-title black-text"> Enviar</span>
					<form class="container" action="PageServlet" method="GET">
						<div class="row">
			...
			<%} %>
			<%
				if(request.getAttribute("iscom") != "isnot"){
			...
			<div class="row">
				...
				<%System.out.println(request.getSession().getAttribute("sharedCom"));
					if (post.get("poster").equals(((Comrade)request.getSession().getAttribute("sharedCom")).getUserName()) || request.getSession().getAttribute("owner").equals(((Comrade)request.getSession().getAttribute("sharedCom")).getUserName())){ %>
					<a href="PageServlet?deletePost=y&indexd=<%out.println(post.get("cont"));%>"><i class = "material-icons small">close</i></a>
					<%} %>
 ...
 %>
						...
						<a
								href="PageServlet?like=y&indexl=<%out.println(post.get("cont"));%>"><i
						...
						<ul id="collapsa2" class="collapsible"
							...
							<li>
								...
								<div class="collapsible-body">
									<div class="card-content">
										<form class="container" action="PageServlet" method="POST">
											<div class="input-field col s12 m6">
